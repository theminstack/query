...
describe('useQuery', () => {
  let mutationFnMock: jest.Mock<any>;

  beforeEach(() => {
    mutationFnMock = jest.fn<any>().mockResolvedValue('foo');
    jest.useFakeTimers();
  });
  ...
  it('should imperatively mutate', async () => {
    const onMutate = jest.fn<any>().mockResolvedValue('context');
    const onSettled = jest.fn<any>().mockResolvedValue(undefined);
