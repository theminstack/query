...
import { type QueryFn, type QueryOptions, useQuery, useStableQueryKey } from './query.js';
...
describe('useQuery', () => {
  let queryFnMock: jest.Mock<any>;

  beforeEach(() => {
    queryFnMock = jest.fn<any>().mockResolvedValue('foo');
    jest.useFakeTimers();
  });
  ...
  it('should cancel the previous query when a new query starts', async () => {
    ...
    act(() => result.current.refetch());
    expect(queryFnMock).toHaveBeenCalledTimes(2);
    expect((queryFnMock.mock.calls.at(-2)?.[0] as any).signal.aborted).toEqual(true);
  ...
  it('should always use the most recent query function instance', async () => {
    const queryFnUpdate = jest.fn<any>().mockResolvedValue('bar');
  ...
  it('should abort fetch on unmount', async () => {
    ...
    expect(queryFnMock).toHaveBeenCalledTimes(1);
    unmount();
    expect((queryFnMock.mock.calls.at(-1)?.[0] as any).signal.aborted).toEqual(true);
  });
  ...
  it('should unmount without error when no query is running', async () => {
    ...
    await waitFor(() => expect(result.current.isFetching).toEqual(false));
    expect(queryFnMock).toHaveBeenCalledTimes(1);
    expect((queryFnMock.mock.calls.at(-1)?.[0] as any).signal.aborted).toEqual(false);
    unmount();
